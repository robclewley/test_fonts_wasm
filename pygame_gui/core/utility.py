...
# TEMP hack
print("TEMP hack in utility to use local folder font file path reference")
...
class FontResource:
    ...
    def __init__(self,
        ...
        # TEMP
        self.font_type_to_use = "freetype" # "pygame"
        print(f"TEMP using {self.font_type_to_use} in FontResource")
    ...
    def load(self):
        ...
        if sys.platform in ('emscripten','wasi',):
            if isinstance(self.location, PackageResource):
                ...
                # TEMP using /tmp bytes
                print("TEMP using /tmp bytes")
        ...
        try:
            if isinstance(location, str):
                # TEMP font loading from string filepath
                print("TEMP font loading from string filepath:", location)
            ...
            elif isinstance(location, PackageResource):
                # TEMP validating that haven't missed removal of any PackageResource attempts
                raise RuntimeError("Found another PackageResource use")
...
#                     self.loaded_font.oblique = self.style['italic']
                ...
                # TEMP font location as bytes
                print(f"TEMP font location = {len(location)} bytes")
                # Not sure if the base64 decode is needed from directly-loaded file bytes
                # file_obj = io.BytesIO(location)
                file_obj = io.BytesIO(base64.standard_b64decode(location))
            ...
            return FileNotFoundError('Unable to load resource with path: ' + str(location))
